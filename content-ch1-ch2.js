...
function loadChapter1() {
    ...
    /* 锁屏密码按钮显式绑定事件（兼容某些不触发 inline onclick 的环境） */
    var keypad = document.getElementById('ch1-keypad');
    if (keypad) {
        var keys = keypad.querySelectorAll('.smp-key');
        for (var ki = 0; ki < keys.length; ki++) {
            (function(btn) {
                var onclickAttr = btn.getAttribute('onclick');
                if (onclickAttr && onclickAttr.indexOf('ch1LockKey') !== -1) {
                    var m = onclickAttr.match(/ch1LockKey\('(.+?)'\)/);
                    if (m) {
                        var keyVal = m[1];
                        btn.addEventListener('click', function() { ch1LockKey(keyVal); });
                        btn.addEventListener('touchstart', function(e) { e.preventDefault(); ch1LockKey(keyVal); });
                    }
                }
            })(keys[ki]);
        }
